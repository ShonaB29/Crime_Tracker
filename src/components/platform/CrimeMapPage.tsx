...
export function CrimeMapPage() {
  ...
  if (isLoading) return <Card className="glass border-white/10 p-6 text-sm text-muted-foreground">Loading map data...</Card>;
  if (error || !data) return <Card className="glass border-white/10 p-6 text-sm text-muted-foreground">Map data unavailable.</Card>;

  return (
    <div className="space-y-4">
      <div>
        <h1 className="font-display text-2xl font-bold text-foreground">Crime Map</h1>
        <p className="text-sm text-muted-foreground">Leaflet-ready hotspot and marker feed for Karnataka district visualization.</p>
      </div>
        ...
        <Card className="glass border-white/10 p-5 xl:col-span-2">
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {data.heatmapPoints.slice(0, 18).map((point, index) => (
              <div key={`${point.district}-${index}`} className="rounded-xl border border-white/10 bg-white/5 p-4">
                ...
                <p className="mt-2 text-xs text-muted-foreground">{point.lat.toFixed(2)}, {point.lng.toFixed(2)}</p>
                <div className="mt-3 h-2 rounded-full bg-white/10">
                  <div className="h-2 rounded-full bg-accent" style={{ width: `${Math.round(point.intensity * 100)}%` }} />
                </div>
              </div>
            ))}
          </div>
