      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
        ...
        <div>
          ...
          <select className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm text-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring">
            {["Constable", "Head Constable", "ASI", "SI", "Inspector", "DSP", "ACP", "DCP"].map(
              (r) => (
          ...
          {pw.length > 0 && (
            <div className="mt-2 space-y-1">
              ...
              <p className="text-xs text-muted-foreground">
                Strength: <span className="text-foreground">{strengthLabel[strength]}</span>
              </p>
            </div>
          )}
        </div>
...
function PrivacySection() {
  const [role, setRole] = useState<"admin" | "senior" | "investigation" | "station">(
    "investigation",
  );
