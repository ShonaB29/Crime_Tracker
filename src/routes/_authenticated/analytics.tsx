...
function AnalyticsPage() {
  ...
  const barSeries: Array<{ label: string; crimes: number; firs: number }> = data.barSeries ?? [];
  const pieSeries: Array<{ name: string; value: number }> = data.pieSeries ?? [];
  ...
  const riskScores: Array<{ district: string; score: number; trend: number }> =
    data.riskScores ?? [];
  const districtComparison: Array<{
    name: string;
    crimes: number;
    solved: number;
    pending: number;
  }> = data.districtComparison ?? [];
  const ageDistribution: Array<{ band: string; value: number }> = data.ageDistribution ?? [];
  const genderDistribution: Array<{ name: string; value: number }> = data.genderDistribution ?? [];
  const anomalyEvents: Array<{
    title: string;
    district: string;
    severity: string;
    signal: string;
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 xl:grid-cols-[1.6fr_1fr]">
        <Card className="glass border-white/10 p-5">
          ...
          <p className="text-xs text-muted-foreground">
            Crimes reported, solved, and pending per month
          </p>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card className="glass border-white/10 p-5">
          ...
          <p className="text-xs text-muted-foreground">
            Top 10 districts — crimes, solved, pending
          </p>
          <div className="mt-4 h-64">
            <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card className="glass border-white/10 p-5">
          ...
          <p className="text-xs text-muted-foreground">
            Composite risk index — higher = more at-risk
          </p>
                ...
                <div className="flex-1 rounded-full bg-white/10 h-2">
                  <div
                    ...
                    style={{
                      width: `${r.score}%`,
                      backgroundColor:
                        r.score > 70 ? "#ff6b6b" : r.score > 45 ? "#ffb86b" : "#58d6c9",
                    }}
                  />
                </div>
        ...
        <Card className="glass border-white/10 p-5">
          ...
          <p className="text-xs text-muted-foreground">
            Districts with statistically elevated crime activity
          </p>
      ...
      <Card className="glass border-white/10 p-5">
        ...
        <p className="text-xs text-muted-foreground mb-4">
          Districts predicted to be high-risk in the next period
        </p>
              ...
              {hotspotPrediction.map((h) => (
                <tr key={h.district} className="border-b border-white/5 text-foreground/90">
                  ...
                  <td className="py-3 pr-6">
                    <span className="rounded-full bg-primary/15 px-2.5 py-1 text-xs font-semibold text-accent">
                      {h.score}
                    </span>
                  </td>
