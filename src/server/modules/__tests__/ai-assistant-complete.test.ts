...
describe("AI Assistant Complete Tests", () => {
  ...
  it("should extract page context and clean queries", async () => {
    ...
    const response = await getHybridAssistantResponse(`open cases\nContext: ${jsonContext}`);
    expect(response.metrics?.query).toContain("status='Open'");

    const lookupResponse = await getHybridAssistantResponse(
      `What is the status of CASE/K001/000042?\nContext: ${jsonContext}`,
    );
    expect(lookupResponse.answer).toContain("CASE/K001/000042");
    expect(lookupResponse.answer).toContain("John Doe");

    const summaryResponse = await getHybridAssistantResponse(
  ...
  it("should route general location queries (e.g. cases in mysore) to SQL correctly", async () => {
    const response = await getHybridAssistantResponse("cases in mysore");
    expect(response.handledBy).toBe("sql");
    expect(response.metrics?.query).toContain("district='mysuru'");
  });

  it("should fall back to RAG for unrecognised queries that contain database details", async () => {
    const response = await getHybridAssistantResponse("crimes involving poison");
    expect(response.handledBy).toBe("rag");
    expect(response.answer).toContain("Poison");
  });

  it("should query Census 2011 demographics via Text-to-SQL", async () => {
    const response = await getHybridAssistantResponse("what is the literacy rate of Dharwad");
    expect(response.handledBy).toBe("sql");
    expect(response.answer).toContain("Dharwad");
    expect(response.metrics?.query).toContain("census_2011");
  });

  it("should execute Pearson correlation analytics for literacy vs crime", async () => {
    const response = await getHybridAssistantResponse("literacy vs crime rate correlation");
  ...
  it("should retrieve Census data from RAG", async () => {
    const response = await getHybridAssistantResponse("Census 2011 demographics Dharwad");
    expect(response.handledBy).toBe("rag");
